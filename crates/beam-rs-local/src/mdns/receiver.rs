...
/// Browse for available beam senders and let user select.
///
...
pub async fn receive_mdns(output_dir: Option<PathBuf>) -> Result<()> {
    println!("Browsing for beam senders on local network...\n");
    ...
    if services.is_empty() {
        println!("\nNo beam senders found on the network.");
        println!("Make sure the sender is running: beam-rs-local send <path>");
        return Ok(());
    }
...
fn prompt_pin() -> Result<String> {
    beam_common::auth::pin::prompt_pin().context("Failed to read PIN")
}
